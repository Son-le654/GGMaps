(function($) {
    ...
    function createSlug(name) {
        const specialChars = {
            ...
            'Ã¢': 'a', 'Ãª': 'e', 'áº¿': 'e', 'á»': 'e', 'á»ƒ': 'e', 'á»…': 'e', 'á»‡': 'e',
            'Ã­': 'i', 'Ã¬': 'i', 'á»‰': 'i', 'Ä©': 'i', 'á»‹': 'i',
            ...
            if(previous_data !== current_guests){
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        let items = res.homestay
                        HT.loadLocation(items)
                        _this.closest('.homestay-item').find('.customer').text(`${res.temp[0].current_guests} khÃ¡ch`);
        ...
        let html = `<div class="popup">
                        ...
                        <div class="status">
                            ...
                            <p class="room">
                                ...
                                <span>${total_rooms[item.total_rooms]} PhÃ²ng</span>
                            </p>
                            <p class="color">
        ...
        $(document).on('click', '#copyBtn', function() {
            ...
            if(link) {
                navigator.clipboard.writeText(link).then(function() {
                    alert('ÄÃ£ copy link: ' + link);
                }).catch(function(error) {
                    console.error('CÃ³ lá»—i khi copy:', error);
                    alert('KhÃ´ng thá»ƒ copy link.');
                });
            }
        });
    };
